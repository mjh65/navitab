...
struct PartPainter
{
    ...
    virtual void RefreshPart(int part, const ImageRectangle* src, const std::vector<Region>& updates) = 0;
};
...
// The Region stucture defines a rectangular area, typically for the purposes
// of updating when only a smaller part of the image has been modified.

struct Region
{
    int left, top, right, bottom;
    Region(int l, int t, int r, int b) : left(l), top(t), right(r), bottom(b) {}
};
...
{
public:
    ImageRectangle(int w, int h) : width(w), height(h) { data.resize(width * height); }
...
class WindowPart : public Callback
{
public:
    enum {
        CANVAS,
...
protected:
    ...
    void Redraw() {
        ...
        painter->RefreshPart(partId, image.get(), dirtyBits);
        dirtyBits.clear();
    }
